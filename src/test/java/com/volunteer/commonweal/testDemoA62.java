...
public class testDemoA62 {
    ...
    public void testA62() throws Exception{
        ...
        ApplyOrganizationApplyData AOAD=new ApplyOrganizationApplyData();

        AOAD.description="我想加入这个组织";
        AOAD.organizationId="5bf1722eccf2b97bc37e303f";

        System.out.println("getActivity验证接口测试");
        try {
            mockMvc.perform(post("/organization/join/apply")
                    .contentType(contentType).content(json(AOAD))
                    .session(session))
                    .andDo(print())
                    .andExpect(status().isOk())
                    .andReturn();
        ...
        System.out.println("getActivity验证接口测试完毕");
