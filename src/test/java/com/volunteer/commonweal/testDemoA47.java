...
//        System.out.println("userSignIn验证接口测试完毕");
    ...
    @Test
    public void testA47_1() throws Exception{
        this.mockMvc = webAppContextSetup(webApplicationContext).build();//加载上下文
        mappingJackson2HttpMessageConverter = new MappingJackson2HttpMessageConverter();
        uData.passport = "adminuser1";
        uData.password = "adminuser1";
        ...
        try {
            mockMvc.perform(post("/activity")
                    ...
                    .andDo(print())
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.name",is("Activity demo")))
                    .andReturn();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    //testA47  user get information from pointed activity
    @Test
    public void testA47_2() throws Exception{
        this.mockMvc = webAppContextSetup(webApplicationContext).build();//加载上下文
        mappingJackson2HttpMessageConverter = new MappingJackson2HttpMessageConverter();
        ...
        System.out.println("userSignIn验证接口测试完毕");
        OnlyIdData OID = new OnlyIdData();
        OID.activityId = "5befff82ccf2b97bc37e3031";
        System.out.println("getActivity验证接口测试");
        try {
            mockMvc.perform(post("/activity")
                    .contentType(contentType).content(json(OID))
                    ...
                    .andDo(print())
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.name",is("彭康之夜")))
                    .andReturn();
        } catch (Exception e) {
            e.printStackTrace();
        }
